    ...
    mainPanel(
      ...
      navbarPage(
        ...
        tabPanel("Health Heatmap",
          sidebarLayout(
            sidebarPanel(
              selectInput("heatmap_country", "Select Country:",
                choices = c("All Countries", sort(unique(alz_df$Country))),
                selected = "All Countries"
              ),
              br(),
              # moved the dataframe here
              tableOutput("heatmap_table"),
              width = 5
            ),
            mainPanel(
...
server <- function(input, output, session) {
  ...
  output$corr_plot <- renderPlotly({
    ...
    p <- ggplot(df, aes(
        ...
        text = paste0(
          ...
          "<br>",
          if(var=="alz_pct") paste0("Alz %: ", round(df$alz_pct,1),"%")
          else if(var=="avg_age") paste0("Avg Age: ", round(df$avg_age,1))
          else paste0("Count: ", df$count)
        )
      )) +
  ...
  filtered_heatmap <- reactive({
    if (input$heatmap_country == "All Countries")
      alz_df
    else
      filter(alz_df, Country == input$heatmap_country)
  })

  heat_data <- reactive({
    df <- filtered_heatmap() |> filter(
      `Alzheimer's Diagnosis` == "Yes"
    )
    if (nrow(df)==0) return(NULL)

    # build diabetes / hypertension / stress tiles
    d1 <- table(df$Gender, df$Diabetes)       |> melt()
    d1$HealthFactor <- "Diabetes"
    d2 <- table(df$Gender, df$Hypertension)   |> melt()
    d2$HealthFactor <- "Hypertension"
    d3 <- table(df$Gender, df$`Stress Levels`) |> melt()
    d3$HealthFactor <- "Stress Levels"

    dat <- bind_rows(d1,d2,d3)
    colnames(dat) <- c("Gender","Condition","Count","HealthFactor")
    dat$HealthFactor <- factor(dat$HealthFactor,
  ...
  output$heatmap_plot <- renderPlot({
    ...
    if (is.null(dat)) {
      ggplot() + annotate("text", x=0.5,y=0.5,
        label="No AD cases for this country") +
      theme_void() + xlim(0,1)+ylim(0,1)
    } else {
      ggplot(dat, aes(
          x=Gender, y=paste(HealthFactor,"-",Condition),
          fill=Count
        )) +
        geom_tile(color="white") +
        geom_text(aes(label=Count),
                  color = ifelse(dat$Count>max(dat$Count)/2,"white","black"),
                  size=4) +
        scale_fill_gradient(low="white", high="steelblue",
                            name="# AD Cases") +
        labs(
          title = paste("AD Cases by Gender & Condition",
            if(input$heatmap_country!="All Countries")
              paste("in", input$heatmap_country) else ""),
          x = "Gender", y = ""
        ) +
        theme_minimal() +
  ...
  output$heatmap_summary <- renderText({
    df <- filtered_heatmap()
    total <- nrow(df)
    ad    <- sum(df$`Alzheimer's Diagnosis`=="Yes")
    paste0("Summary: ", ad, " AD cases out of ", total,
           " (", round(ad/total*100,1), "%).")
  })

  output$heatmap_table <- renderTable({
    if (input$heatmap_country=="All Countries") {
      alz_df %>%
        group_by(Country) %>%
        summarise(
          Total            = n(),
          `AD Cases`       = sum(`Alzheimer's Diagnosis`=="Yes"),
          `Percent (%)`    = round(`AD Cases`/Total*100,1),
          .groups="drop"
        ) %>%
        arrange(desc(`AD Cases`))
    } else {
      subs <- filtered_heatmap()
      adc  <- filter(subs, `Alzheimer's Diagnosis`=="Yes")
      if (nrow(adc)==0) {
        data.frame(Message = paste("No AD cases in", input$heatmap_country))
      } else {
        adc %>%
