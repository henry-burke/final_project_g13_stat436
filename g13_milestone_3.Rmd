...
alz = read_csv("https://uwmadison.box.com/shared/static/a2a74zejbdt5ywk7pxsvsw06gsypi8to.csv") |>
  ...
  # mutate(
  #   `Alzheimer’s Diagnosis` = if_else(`Alzheimer's Diagnosis` == "Yes", 1, 0)
  # )

# Get country geographical data
countries_geo = ne_countries(scale = "medium", returnclass = "sf")
# Properly rename for sf objects
...
# Get coordinates for the circles
...
# Join coordinates with metrics data
circles_data = country_metrics |>
  inner_join(countries_coords, by = "Country")

# Join with geographical data for polygons
map_data = countries_geo |>
  inner_join(country_metrics, by = "Country")

# Create color palettes
diabetes_palette = colorNumeric(palette = "Reds", domain = range(country_metrics$diabetes_rate))
age_palette = colorNumeric(palette = "YlOrBr", domain = range(country_metrics$avg_age))

# Create the combined map directly without creating intermediate maps
combined_map <- leaflet() |>
  ...
  # Add circle markers for Alzheimer's rates on both layers
  addCircleMarkers(
    data = circles_data,
    lng = ~X,
    lat = ~Y,
    radius = ~sqrt(alzheimers_rate) * 15,
    stroke = TRUE,
    color = "black",
    weight = 1,
    fillColor = "purple",
  ...
  # Add appropriate legends for each layer
  addLegend(
    position = "bottomright",
    pal = diabetes_palette,
    values = seq(min(country_metrics$diabetes_rate), max(country_metrics$diabetes_rate), length.out = 5),
    title = "Diabetes Rate",
  ...
  addLegend(
    position = "bottomright",
    pal = age_palette,
    values = seq(min(country_metrics$avg_age), max(country_metrics$avg_age), length.out = 5),
    title = "Average Age (years)",
    opacity = 0.7,
    group = "Average Age"
  ) |>
  # Add legend for Alzheimer's circles (shown with both layers)
  addLegend(
    position = "bottomleft",
    colors = "purple",
    labels = "Alzheimer's Rate (circle size)",
    opacity = 0.8,
